...
public class CourseController {
	...
	@GetMapping("/courses-panel/{numPage}")
	public String coursesPanelnumPage(Model model, @PathVariable int numPage) throws SQLException {
		...
		Page<Course> coursesListPage = courseService.findAll(pageable);
		List<Course> coursesList = coursesListPage.getContent();
	...
	public String coursesPanel(Model model) throws SQLException {
		...
		model.addAttribute("filters", filterPair);
		model.addAttribute("delete", true);
		return "course-grid-admin";
	}
	@GetMapping("/courses/{numPage}")
	public String coursesPage(Model model, @PathVariable int numPage) throws SQLException {
		List<String> filters = courseService.findAllCategories();
		List<String[]> filterPair = new ArrayList<>();
		List<String[]> courseInfo = new ArrayList<>();
		List<Map<String, Object>> coursesModel = new ArrayList<>();
	...
	public String courses(Model model) throws SQLException {
		...
		for (String[] aux : courseInfo) {
			...
			for (int i = 0; i < 5; i++) {
				Map<String, Object> star = new HashMap<>();
				star.put("filled", i < Math.floor(valoration));
				stars.add(star);
			}
			courseData.put("0", aux[0]);
			courseData.put("1", aux[1]);
	...
	public String courseSearchPageAdmin(Model model, @RequestParam String name)
			throws SQLException {
		List<String> filters = courseService.findAllCategories();
		List<String[]> filterPair = new ArrayList<>();
		List<String[]> courseInfo = new ArrayList<>();
		...
		int page = 0;
		Pageable pageable = PageRequest.of(page, 10);
		Page<Course> coursesListPage = courseService.findByNameContains(name, pageable);
		List<Course> coursesList = coursesListPage.getContent();
		Double valoration = 0.0;
		for (int i = 0; i < filters.size(); ++i) {
			String[] aux = new String[2];
	...
	public String courseSearchPage(Model model, @RequestParam String name)
			throws SQLException {
		List<String> filters = courseService.findAllCategories();
		List<String[]> filterPair = new ArrayList<>();
		List<String[]> courseInfo = new ArrayList<>();
	...
	@GetMapping("/course-search/{numPage}")
	public String courseSearch(Model model, @RequestParam String name, @PathVariable int numPage)
			throws SQLException {
		List<String> filters = courseService.findAllCategories();
		List<String[]> filterPair = new ArrayList<>();
		List<String[]> courseInfo = new ArrayList<>();
