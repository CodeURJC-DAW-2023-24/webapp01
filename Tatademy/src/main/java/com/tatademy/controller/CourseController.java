...
public class CourseController {
	...
	@GetMapping("/leaveCourse/{courseId}")
	public String leaveCourse(Model model, HttpServletRequest request, @PathVariable Long courseId) {
		Principal principal = request.getUserPrincipal();
		User user = userService.findByEmail(principal.getName());
		List<Course> currentCurses = user.getCourses();
		Optional<Course> course = courseService.findById(courseId);
		course.ifPresent(currentCurses::remove);
		user.setCourses(currentCurses);
		userService.save(user);
		// model.addAttribute("joined", true);
		return "redirect:/course-details/" + courseId;
	...
	public String saveReview(Model model, RedirectAttributes redirectAttrs, @PathVariable Long courseId,
		...
		if (rateReview > 5 || rateReview < 1) {
			...
			newReview.setCreationDate(Calendar.getInstance());
			// Save the review
